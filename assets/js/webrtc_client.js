...
const SV_ICE_SERVERS = [
    ...
    // TURN server for symmetric NAT traversal (required for mobile/cellular connections)
    // Replace the placeholders below with your actual TURN server details (e.g., from Twilio, Metered.ca, or a self-hosted CoTURN setup)
    {
        urls: [
...
class WebRTCClient {
    ...
    async initiateCall(callType) {
        if (!this.config.orderId) throw new Error('WebRTCClient: orderId is required to start a call');
        ...
        if (!res.success) {
            throw new Error('[WebRTCClient] initiate_call failed: ' + (res.error || res.message || 'unknown'));
        }
    ...
    async handleIncomingCall(callData) {
        if (!this.config.orderId && !callData.order_id) {
            throw new Error('WebRTCClient: orderId is required to answer a call');
        }
        if (!this.config.orderId) {
            this.config.orderId = callData.order_id;
        }
        ...
        if (!res.success) {
            throw new Error('[WebRTCClient] answer_call failed: ' + (res.error || res.message || 'unknown'));
        }
    ...
    async _sendIce(candidate) {
        ...
        body.append('call_session_id', this.callSessionId);
        body.append('candidate',       JSON.stringify(candidate)); // matches local API field name
    ...
    async _doPoll() {
        ...
        /* Update our callSessionId if we get it from server (first poll) */
        if (res.call_session_id && !this.callSessionId) {
            this.callSessionId = res.call_session_id;
        }

        /* ── For caller: detect when sdp_answer arrives directly in poll response ── */
        if (this.isCaller && res.sdp_answer && this.peerConnection &&
            !this.peerConnection.remoteDescription) {
            try {
                const answerSdp = JSON.parse(res.sdp_answer);
                await this.peerConnection.setRemoteDescription(new RTCSessionDescription(answerSdp));
                console.log('[WebRTCClient] Remote description (answer) set from poll');
                await this._flushIceQueue();
            } catch (err) {
                console.error('[WebRTCClient] setRemoteDescription from poll failed:', err);
            }
        }
        ...
        /* ── Handle status field (our local API returns 'status') ─────── */
        const callStatus = res.status || res.call_status;

        if (callStatus === 'active' && !this._connectedFired) {
        ...
        if ((callStatus === 'ended' || callStatus === 'missed') && !this._ended) {
            this._ended = true;
            this._cleanup();
            this.config.onCallEnded(0);
        }

        if (callStatus === 'declined' && !this._ended) {
            this._ended = true;
            this._cleanup();
            this.config.onCallDeclined();
        }
