...
// Check if order_id exists in manual_tasks table
$stmt = $conn->prepare("SELECT mt.id, v.business_name, v.name FROM manual_tasks mt LEFT JOIN vendors v ON mt.assigned_vendor_id = v.id WHERE mt.id = ?");
if ($stmt) {
    ...
    if ($res) {
        $is_offline = 1;
        $partner_name = $res['business_name'] ?: ($res['name'] ?: "Partner");
    }
    $stmt->close();
}

if (!$is_offline) {
    // Check shop order assignment
    $stmt2 = $mainConn->prepare("SELECT ova.vendor_id, v.business_name, v.name FROM order_vendor_assignments ova JOIN vendors v ON ova.vendor_id = v.id WHERE ova.order_id = ? LIMIT 1");
    if ($stmt2) {
        $stmt2->bind_param("i", $order_id);
        $stmt2->execute();
        $res2 = $stmt2->get_result()->fetch_assoc();
        if ($res2) {
            $partner_name = $res2['business_name'] ?: ($res2['name'] ?: "Partner");
        }
        $stmt2->close();
    }
}
...
<body>
    ...
    <div class="call-container">
        ...
        <div class="calling-state-overlay" id="connectingOverlay">
            ...
            <div class="caller-title"><?= htmlspecialchars($partner_name) ?></div>
    ...
    <script>
        ...
        async function logCallOutcome(messageText) {
            try {
                ...
                // Call save_admin_message.php
                await fetch('../vendor/ajax/save_admin_message.php', {
                    method: 'POST',
                    body: fd
                });
            } catch (e) {
        ...
        function closeCallWindow() {
            window.close();
            setTimeout(() => {
                window.location.href = '/admin/tracking.php';
            }, 500);
        }
        ...
        const client = new WebRTCClient({
            ...
            onCallDeclined() {
                ...
                logCallOutcome(`📞 ${typeText} call declined`).then(() => {
                    alert('Call declined by partner.');
                    closeCallWindow();
                });
            },
            onCallMissed() {
        ...
        async function initCall() {
            ...
            fetch('../vendor/ajax/ensure_webrtc_order.php?id=' + ORDER_ID).catch(e => {
            ...
            if (ACTION === 'answer' && INCOMING_CALL_ID > 0) {
                ...
                // Dialing out
                document.getElementById('txtStatus').innerText = 'Calling partner...';
